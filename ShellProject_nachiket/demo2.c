/* 
   tsh - A tiny shell program with job controls
   Nachiket Trivedi
   201401047    
 */
#include <stdio.h>
...
void eval(char *cmdline) 
{
   sigset_t sigbits;
    ...
    if(builtin_cmd(argv)!=1){   /*Check if command to be executed is fg or bg*/
        if(sigemptyset(&sigbits)==-1){ /* Make the signal bits empty*/
            unix_error("sigemptyset error"); 
        }
        
        if(sigaddset(&sigbits,SIGCHLD)){   /* Add the sigchld bit to the set */
            unix_error("sigaddset error");
        }
        
        if(sigprocmask(SIG_BLOCK,&sigbits,NULL)==-1){  /* Mask the sigchld signal */
            unix_error("sigprocmask error");
        }

        pid=fork();
        if(pid==0){
            if(sigprocmask(SIG_UNBLOCK,&sigbits,NULL)==-1){     /* Unblock sigchld for child process */
                unix_error("sigprocmask error");
            }

            if(setpgid(0,0)==-1){ /* Set the group id of the command to be executed */
            ...
            if(sigprocmask(SIG_UNBLOCK,&sigbits,NULL)==-1){ /* Unblock sigchld for parent process */
                unix_error("sigprocmask error");
            }
            if(bg==0){
                waitfg(pid);    /* Wait untill fg process is completed */
            }
        }
    }
    return;
...
void do_bgfg(char **argv) 
{
    ...
    else
    {
        ...
        if((val==0 || count!=strlen(argv[1])) && strcmp(argv[0],"fg")==0)
            printf("fg: argument must be a PID or %%jobid \n");
        else if((val==0 || count!=strlen(argv[1])) && strcmp(argv[0],"bg")==0)
            printf("bg: argument must be a PID or %%jobid \n");
        else
        {
            if (argv[1][0]=='0')
            {
                jd=getjobjid(jobs,val);   
                if(jd==NULL)
                {
...
void sigchld_handler(int sig) 
{
    pid_t cpid;                                                                //Stores the child pid
    int status;                                                                     //Status variable

    while((cpid = waitpid(-1, &status, WNOHANG|WUNTRACED)) > 0)
    {               //Get the child pid in the loop
        struct job_t *jd = getjobpid(jobs, cpid);                              //Get job detail of the child
        if(!jd)
        {                                                                    //If no job
            printf("((%d): No such child", cpid);                              //Throw error
            return;
        }
        ...
        {                                                     //If stopped
            ...
            printf("Job [%d] (%d) stopped by signal 20\n",jd->jid, cpid);      //print the message
        }

        else if(WIFSIGNALED(status))
        {                                               //If signalled
            deletejob(jobs, cpid);                                             //Delete job from jobs list
            printf("Job [%d] (%d) terminated by signal 2\n", jd->jid, cpid);
        }

        else if(WIFEXITED(status))
        {                                                 //If exited
            deletejob(jobs, cpid);                                             //Delete from jobs list
        }
